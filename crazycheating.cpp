...
void solve(int T)
{
	int n; cin >> n;
	vpii a(n);
	fr(i,0,n) {cin >> a[i].first; cin >> a[i].second;};
	reverse(a.begin(), a.end());
	vvi dp(n+1,vi(2,0));

	fr(i,1,n+1)
	{
		setmax(dp[i][0], dp[i-1][0]);
		setmax(dp[i][0], dp[i-1][1]);
		int pos = max( i- a[i-1].second - 1,(long long) 0);
		setmax(dp[i][1],dp[pos][1] + a[i-1].first);
		setmax(dp[i][1],dp[pos][0] + a[i-1].first);
		setmax(dp[i][1],dp[i-1][1]);
		setmax(dp[i][1],dp[i-1][0]);
