...
int rec (int i, int mask, int &n, vvi &dp, vvi &cost)
{
	if (i == n)
	{
		return 0;
	...
	fr(j,0,n)
	{
		 if (mask & ( 1 << j) )
		 {
			 setmin(ans,cost[j][i] +  rec(i+1, (mask ^ (1 << j)),n,dp,cost) );
		 }
...
{
	int n; cin >>n ;
	vvi costs(n+1,vi(n+1));
	fr(i,0,n) {fr(j,0,n) {cin >> costs[i][j];}}

	// dp[i][mask] = min cost to assign people from job number i till job n, mask: available people
	vvi dp(n+1,vi((1 << n),-1));
	cout << rec(0,(1 << n) - 1,n,dp,costs) << '\n';
}
