...
bool isValid(int i, int j, vvi &vis, int n, int m)
{
...
void dfs(vs&grid,vvi&vis,int i, int j,int n,int m)
{
	if (!isValid(i,j,vis,n,m) || grid[i][j] == '#') return;
	vis[i][j] = 1;

	for (auto [x,y] : moves)
	{
		dfs(grid,vis,i+x,j+y,n,m);
	}
