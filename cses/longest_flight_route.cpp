...
bool toposort_dfs(vvi&adj, vi &vis, vi &currSearch, vi&toposorted,int v)
{
	if (currSearch[v]) return true;
	...
	for (auto u : adj[v])
	{
		if (toposort_dfs(adj,vis,currSearch,toposorted,u)) return true;
...
vi toposort(vvi &adj, int n)
{
	...
	fr(i,0,n)
	{
		if (toposort_dfs(adj,vis,currSearch,toposorted,i)) return {};
	}
	return toposorted;
}
...
void solve(int T)
{
	...
	vvi adj(n,vi());
	vvi fake_adj(n,vi());
	fr(i,0,m) {int a, b; cin >> a >> b; a--; b--; adj[a].pb(b); fake_adj[b].push_back(a);}
	vpii dp (n,pii(0,-1));
	vi vis(n,0);
	vi fake_vis(n,0);
	dfs(fake_adj,fake_vis,n-1);
	...
	{
		if (!fake_vis[v]) continue;
		for (auto u : fake_adj[v])
		{
			if (!fake_vis[u]) continue;
			if (dp[v].ff + 1 > dp[u].ff)
			{
				dp[u].ff = dp[v].ff + 1;
				dp[u].ss = v;
			}
