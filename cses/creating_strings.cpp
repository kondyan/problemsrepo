...
set<string> res;
void rec(map<char,int> curr_characters, string curr_string, int n)
{
	if (curr_string.size() == n)
	{
		res.insert(curr_string);
		return;
	}
	for (auto [character, count]: curr_characters)
	{
		string copy_curr_string = curr_string;
		map<char,int> copy_curr_characters = curr_characters;
		if (count!=0)
		{
			copy_curr_characters[character]--;
			copy_curr_string.push_back(character);
			rec(copy_curr_characters, copy_curr_string, n);
		}
