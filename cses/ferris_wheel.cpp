...
void solve(int T)
{
	int n, x; cin >> n >> x;
	vi weights(n);
	unordered_map<int,int> weights_left;
	fr(i,0,n){cin >> weights[i];weights_left[weights[i]]++;}
	...
	fr(i,0,n)
	{
		if (weights_left[weights[i]] <= 0) continue;
		weights_left[weights[i]]--;
		int search = x - weights[i];

		auto it = lower_bound(weights.begin() , weights.end(), search,greater<int>());
		if (it != weights.end())
		{
			int found = *it;
			if (weights_left[found] >= 1) weights_left[found]--;
		}
		ans++;
	}
	cout << ans << '\n';
