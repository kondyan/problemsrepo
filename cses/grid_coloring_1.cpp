...
int flag = 0;
vector<vector<char>> bfs(vector<vector<char>> & grid, vvi& visited, int n, int m) {

	vector<vector<char>> res(n+1, vector<char>(m+1, '.'));
	queue<pii> q;
	int flag = 0;
	q.push({1,1});
	while (!q.empty())
	{
		...
		{
			if ( color == grid[i][j] || color == res[i-1][j] || color == res[i][j-1]) continue;
			curr_color = color;
			break;
		}
		if (curr_color == '.')
...
void solve(int T)
{
	...
	vector<vector<char>> res = bfs(grid,visited,n,m);
