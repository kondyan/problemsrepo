...
bool dfs(vvi &adj, vi&vis, vi&currSearch, vi&par, int v, int last)
{
	if (currSearch[v]) {cycle_end = v; par[v] = last;return true;}
	...
	for (auto u : adj[v])
	{
		if (dfs(adj,vis,currSearch,par,u,v)) return true;
	}
	currSearch[v] = 0;
	return false;
}
...
vi toposort(vvi &adj, int n)
{
	...
	fr(i,0,n)
	{
		if (!vis[i] && dfs(adj,vis,currSearch,par,i,-1))
		{
			// return cycle
			vi cycle;
			cycle.pb(cycle_end);
			...
			if (cycle[0] == cycle[cycle.size() -1 ] && cycle.size() > 2)return cycle;
		};
	}
...
void solve(int T) {
	...
	int n, m; cin >> n >> m;
	vpii degrees(n,pii(0,0));
	vvi adj(n,vi());
	fr(i,0,m) {int a, b; cin >> a  >> b; a-- ;b--; adj[a].pb(b);degrees[a].ss++; degrees[b].ff++;}
