...
vi dijkstra(vvpii &adj, int n, int k)
{
	vi dist(n,-1);
	vi vis(n,0);
	...
	while (!pq.empty() || routes.size() != k)
	{
		...
		if (v == n-1) routes.push_back(distance);

		for (auto [u,c] : adj[v])
		{
			pq.push({distance + c, u});
		}
	}
	return routes;
}
...
void solve(int T)
{
	...
	int n, m, k; cin >> n >> m >> k;
	vvpii adj(n,vpii());
	fr(i,0,m) {int a, b, c; cin >> a >> b >> c; a--; b--; adj[a].pbp(b,c);}

	vi ans = dijkstra(adj,n,k);
	for (auto el : ans)
	{
		cout << el << ' ';
	}
