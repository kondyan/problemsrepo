...
void solve(int T)
{
  int n, m ,k ; cin >> n >> m >> k;
	vi a1(n);
	map<int,int> a2;
	fr(i,0,n) {cin >>a1[i];} fr(i,0,m) {int x; cin >> x; a2[x]++;}
	...
	{
		int val = el-k;
		auto it = a2.lower_bound(val);
		if (it == a2.end()) continue;
		pair<int,int> apartment = *it;

		if (el + k >= apartment.first)
		{
			ans++;
			a2[apartment.first]--;
			if (a2[apartment.first] <= 0) a2.erase(apartment.first);
		}
	}
	cout << ans << '\n';
}
