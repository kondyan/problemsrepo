...
void dijsktra(vvpii &adj, vi&dp_min, vi&dp_max, vi&dp_ways, int n)
{
	vi vis(n,0);
	vi dist(n,inf);
	...
	while (!pq.empty())
	{
		...
		for (auto [u, c] : adj[v])
		{
			if (d + c == dist[u])
			{
				setmin(dp_min[u], dp_min[v] + 1);
				setmax(dp_max[u], dp_max[v] + 1);
				dp_ways[u] = (dp_ways[u] + dp_ways[v])%mod;
			} else if (d + c < dist[u])
			{
				dp_min[u] = dp_min[v] + 1;
				dp_max[u] = dp_max[v] + 1;
				dp_ways[u] = (dp_ways[v]) % mod;
				pq.push({c + d, u});
				dist[u] = d + c;
...
void solve(int T)
{
	...
	dp_min[0] = 0;
	dp_ways[0] = 1;
	vvpii adj(n,vpii()); fr(i,0,m) {int a, b, c; cin >> a >> b >> c; a--; b--; adj[a].pbp(b,c);}

	// min distance, amount of min distant ways, min num of edges on min dist path, max num of edges on min dist path


	dijsktra(adj,dp_min,dp_max,dp_ways,n);
