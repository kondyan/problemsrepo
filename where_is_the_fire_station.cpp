...
void solve(int T)
{
	...
	fr(k,3,n)
	{
		...
		fib[k] = (fib[k] + fib[k-3]) % mod;
	}

	int left = i;
	int right = n-j-1;

	int first =( fib[j-i-1] * left) % mod;
	int second = (fib[j-i-1] * right) % mod;

	int third = 0;
	if (j-i > 1)
	{
		third = (fib[j-i-2]*left) % mod;
		third = (third * right) %mod;

	}
	int ans = (fib[j-i] + first) % mod;
	ans = (ans + second)%mod;
	ans = (ans + third)%mod;
