...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 0; j <= i; j++)
        {
            // chance of getting j heads using i coins

            // first consider the probability of getting tail on current step
            // chance of getting j heads with all previous coins * chance of getting tail now
            dp[i][j] = dp[i-1][j] * (1 - c[i-1]);
            if (j > 0)
            {
                // probability of getting head on current step with the i-th coin
                // add
                dp[i][j] += dp[i-1][j-1] * c[i-1];
