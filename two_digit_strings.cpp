...
void solve(int T)
{
	...
	vi pref_a(str_a.size()+1,0);
	vi pref_b(str_b.size()+1,0);


	fr(i,1,str_a.size()+1)
	{
		pref_a[i] = pref_a[i-1] + (str_a[i-1]-'0');
	}

	fr(i,1,str_b.size()+1)
	{
		pref_b[i] = pref_b[i-1] + (str_b[i-1]-'0');
	}

	vvi dp(str_a.size()+1,vi(str_b.size()+1,0));
