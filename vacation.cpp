...
void solve()
{
    ...
    for (int i = 2; i <= n; i++)
    {
        setmax(dp[i][0], arr[i][0] + max(dp[i-1][1], dp[i-1][2]));
        setmax(dp[i][1], arr[i][1] + max(dp[i-1][0], dp[i-1][2]));
        setmax(dp[i][2], arr[i][2] + max(dp[i-1][0], dp[i-1][1]));
    }
