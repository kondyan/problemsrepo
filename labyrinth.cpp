...
int dijkstra(vector<vector<char>> &grid, int curr_left, int curr_right,int x, int y, int n, int m, int left_lim, int right_lim)
{
	...
	while (!pq.empty())
	{
		...
		if (grid[curr_x][curr_y] == '*') continue;
		if (dist[curr_x][curr_y] != inf) continue;
		dist[curr_x][curr_y] = left_moves;
		ans++;

		if (left_moves < left_lim)
		{
			if (curr_y-1 >= 0 )
			{
				if (grid[curr_x][curr_y-1] != '*')
				{
					pq.push({left_moves+1,{curr_x,curr_y-1}});
				}
			}
		}

		if (curr_y-y + left_moves < right_lim)
		{
			if (curr_y+1 < m)
			{
				if (grid[curr_x][curr_y+1] != '*')
				{
					pq.push({left_moves, {curr_x,curr_y+1}});
				}
			}
		}
		...
		if (curr_x + 1 < n)
		{
			...
			{
				pq.push({left_moves,{curr_x+1,curr_y}});
			}
		}
		if (curr_x - 1 >= 0 )
		{
...
void solve()
{
	...
	int r, c; cin >> r >> c; r--; c--;
	int left_lim,right_lim; cin >> left_lim >> right_lim;
	...
	int ans = dijkstra(grid,0,0,r,c,n,m,left_lim,right_lim);
	cout << ans << '\n';
}
